...
#include <fstream>
#include <vector>
#include "/home/cpsl/SDK_ws/src/offboard/include/offboard/offboard_ctrl_squared_data.h"
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "offboard_ctrl_squared_data");
    ros::NodeHandle nh;

    // Define the file name
    std::string vicon_posi_file_name = "vicon_posi_data.txt";
    std::string vicon_orient_file_name = "vicon_orient_data.txt";
    std::string ekf_orient_file_name = "ekf_orient_data.txt";
    std::string imu_ang_vel_file_name = "imu_ang_vel_data.txt"; 
    std::string imu_linear_accel_file_name = "imu_linear_accel_data.txt";

    // Create an ofstream object in append mode
    std::ofstream vicon_posi_file(vicon_posi_file_name, std::ios::app);
    std::ofstream vicon_orient_file(vicon_orient_file_name, std::ios::app);
    std::ofstream ekf_orient_file(ekf_orient_file_name, std::ios::app);
    std::ofstream imu_ang_vel_file(imu_ang_vel_file_name, std::ios::app);
    std::ofstream imu_linear_accel_file(imu_linear_accel_file_name, std::ios::app);

    // Check if the file was successfully opened
    if (!vicon_posi_file) {
        ROS_ERROR("Unable to open file %s", vicon_posi_file_name.c_str());
        return 1;
    }

    if (!vicon_orient_file) {
        ROS_ERROR("Unable to open file %s", vicon_orient_file_name.c_str());
        return 1;
    }

    if (!ekf_orient_file) {
        ROS_ERROR("Unable to open file %s", ekf_orient_file_name.c_str());
        return 1;
    }

    if (!imu_ang_vel_file) {
        ROS_ERROR("Unable to open file %s", imu_ang_vel_file_name.c_str());
        return 1;
    }

    if (!imu_linear_accel_file) {
        ROS_ERROR("Unable to open file %s", imu_linear_accel_file_name.c_str());
        return 1;
    }
    ...
    current_pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("mavros/local_position/pose", 10, 
        [&ekf_orient_file](const geometry_msgs::PoseStamped::ConstPtr& msg) {
            currentPose_cb(msg, ekf_orient_file);
        });


    vicon_pose_sub = nh.subscribe<geometry_msgs::TransformStamped>("vicon/x500_1/x500_1", 10, 
        [&vicon_posi_file, &vicon_orient_file](const geometry_msgs::TransformStamped::ConstPtr& msg) {
            viconPose_cb(msg, vicon_posi_file, vicon_orient_file);
        });


    imu_sub = nh.subscribe<sensor_msgs::Imu>("sensor_msgs/Imu", 10, 
        [&imu_ang_vel_file, &imu_linear_accel_file](const sensor_msgs::Imu::ConstPtr& msg) {
            imu_cb(msg, imu_ang_vel_file, imu_linear_accel_file);
        });
...
void currentPose_cb(const geometry_msgs::PoseStamped::ConstPtr& msg, std::ofstream& ekf_orient_file) {
    // local_position stores the current position and orientation of the drone
    current_pose = *msg;


    // Write the received message data to the file
    ekf_orient_file << ("%.2f,%.2f,%.2f", current_pose.pose.orientation.x, current_pose.pose.orientation.y, current_pose.pose.orientation.z) << std::endl;
...
void viconPose_cb(const geometry_msgs::TransformStamped::ConstPtr& msg, std::ofstream& posi_file, std::ofstream& orient_file) {
    ...
    // Write the received message data to the file
    posi_file << ("%.2f,%.2f,%.2f", vicon_pose.transform.translation.x, vicon_pose.transform.translation.y, vicon_pose.transform.translation.z) << std::endl;
    orient_file << ("%.2f,%.2f,%.2f,%.2f", vicon_pose.transform.rotation.w, vicon_pose.transform.rotation.x, vicon_pose.transform.rotation.y, vicon_pose.transform.rotation.z) << std::endl;
    ROS_INFO("Data successfully appended to the file");
...
void imu_cb(const sensor_msgs::Imu::ConstPtr& msg, std::ofstream& imu_ang_vel_file, std::ofstream& imu_linear_accel_file) {
    // Write the received message data to the file
    imu_ang_vel_file << ("%.2f,%.2f,%.2f", msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z) << std::endl;
    imu_linear_accel_file << ("%.2f,%.2f,%.2f", msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z) << std::endl;
}
...
bool position_control(ros::Time& last_request, mavros_msgs::CommandBool& arm_cmd) {
    ...
    if(isAtPosition(target[0], target[1], target[2], 0.1, 0.15)) {
        if(state == 0) {
            ROS_INFO("Reached Landing Position. Prepare to disarm");

            return true;
        }
    }

    if(isAtPosition(target[0], target[1], target[2], 0.1, 0.15) && (ros::Time::now() - last_request > ros::Duration(180.0))) {
